...
class FeatureAgent(MahjongGBAgent):
    ...
    OFFSET_OBS = {
        'SEAT_WIND' : 0,  # Seat wind.
        'PREVALENT_WIND' : 1,  # Round wind.
        'HAND' : 2,  # Own hand, encoded by duplicate count.
        
        'DISCARD' : 6,  # Discards for all four relative seats.
        'HALF_FLUSH' : 22,
        'CHI':22,  # Exposed chi melds for all four relative seats.
        'PENG':38,  # Exposed peng melds for all four relative seats.
        'GANG':54,  # Exposed or visible kong melds for all four relative seats.
        'SHANTEN':70  # Own shanten number.
    }
    OFFSET_ACT = {
        'Pass' : 0,
        'Hu' : 1,
    ...
    def _hand_embedding_update(self):
        ...
        for i in range(4):
            for tri in packs[i]:
                d=defaultdict(int)
                if tri[0] == 'CHI':
                    # A chi meld is stored by its middle tile.
