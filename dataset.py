...
class MahjongGBDataset(Dataset):
    ...
    def __init__(self, begin = 0, end = 1, augment = False):
        import json
        with open('国标麻将作业/code/data/count.json') as f:
        ...
        for i in range(self.matches):
            ...
            if augment:
                d=np.load('国标麻将作业/code/data/augmented_data/%d_augmented_%d.npz'%(i+self.begin,feature_num))
            else:
                d = np.load('国标麻将作业/code/data/unarg_data/%d.npz' % (i + self.begin))
            for k in d:
